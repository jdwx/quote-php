...
class StringParser {
    ...
    public static function parseStringEx( string $i_stLine, bool $i_bSingleQuotes = true,
                                          bool   $i_bDoubleQuotes = true,
                                          bool   $i_bBackquotes = true ) : ParsedString {
        $pst = self::parseString( $i_stLine, $i_bSingleQuotes, $i_bDoubleQuotes, $i_bBackquotes );
        if ( $pst instanceof ParsedString ) {
            return $pst;
        }
        throw new \RuntimeException( "Unable to parse string ({$pst}): {$i_stLine}" );
    }
